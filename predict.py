...
def perform_regression(input_file, data_file):
    ...
    for activation in ACTIVATION_TYPES:
        ...
        for l1 in range(L1[0],L1[1],L1[2]):
            t0 = time.time()
            mlp = MLPRegressor(algorithm='l-bfgs', hidden_layer_sizes=(l1,),
                        max_iter=200, shuffle=True, random_state=1,
                        activation=activation, alpha=1.1)
                           
            scores = cross_validation.cross_val_score(mlp, X_train, y_train, cv=10)
            fp.write("Activation: %s, Network : (%d)\n" % (activation, l1))
            fp.write("Accuracy: %0.2f (+/- %0.2f)\n" % (scores.mean(), scores.std()))
    ...
    for activation in ACTIVATION_TYPES:
        ...
        # for l1 in range(L1[0],L1[1]):
            for l2 in range(L2[0], L2[1], L2[2]):
                ...
                t0 = time.time()
                mlp = MLPRegressor(algorithm='l-bfgs', hidden_layer_sizes=(l1,l2,),
                           max_iter=200, shuffle=True, random_state=1,
                           activation=activation, alpha=1.1)
        ...
        for l1 in range(L1[0],L1[1],L1[2]):
            ...
            for l2 in range(L2[0], L2[1], L2[2]):
                for l3 in range(L3[0], L3[1], L3[2]):
                    ...
                    t0 = time.time()
                    mlp = MLPRegressor(algorithm='l-bfgs', hidden_layer_sizes=(l1,l2,l3,),
                           max_iter=200, shuffle=True, random_state=1,
                           activation=activation, alpha=1.1)
...
def load_data_prediction(out_dir):
    spath = out_dir + '/data'
    x_train_path = spath + '/x_train.txt'
...
def master(out_dir):
    # load data
    X_train, X_test, y_train, y_test = load_data_prediction(out_dir)

    # load scalers
    s_path = out_dir + '/scaler/scaler.pkl'
    scaler = joblib.load(s_path)

    # load models
    log_path = out_dir + '/mlp_l/mlp.pkl'
    model_l = joblib.load(log_path)
    
    relu_path = out_dir + '/mlp_r/mlp.pkl'
    model_r = joblib.load(relu_path)

    tanh_path = out_dir + '/mlp_t/mlp.pkl'
    model_t = joblib.load(tanh_path)
    ...
    header = np.hstack(['num_days', 'pd', 'ps', 'temp', 'per_leaching', 'logistic_pred', 'relu_pred', 'tanh_pred']).reshape(1,-1)

    data = np.hstack([x, y_test.reshape(-1,1), pred_l.reshape(-1,1), pred_r.reshape(-1,1), pred_t.reshape(-1,1)])

    sname = out_dir + '/results/'
    if not os.path.exists(sname):
        os.makedirs(sname)
    sname = sname + 'response_test.txt'
    np.savetxt(sname, header, fmt='%s', delimiter=' & ')
    with open(sname, 'a') as fp:
        np.savetxt(fp, data, fmt='%.2f', delimiter=' & ')

    pred_l = model_l.predict(X_train)
    pred_r = model_r.predict(X_train)
    pred_t = model_t.predict(X_train)

    x = scaler.inverse_transform(X_train)

    data = np.hstack([x, y_train.reshape(-1,1), pred_l.reshape(-1,1), pred_r.reshape(-1,1), pred_t.reshape(-1,1)])

    sname = out_dir + '/results/'
    if not os.path.exists(sname):
        os.makedirs(sname)
    sname = sname + 'response_train.txt'
    np.savetxt(sname, header, fmt='%s', delimiter=' & ')
    with open(sname, 'a') as fp:
        np.savetxt(fp, data, fmt='%.2f', delimiter=' & ')

    sname = out_dir + '/results/model_details.tsv'
    fp = open(sname, 'w')
    for model in (model_l, model_r, model_t):
        fp.write(model.get_params()['activation'])
        fp.write('\n')
        #print model.get_params()
        fp.write('Best Model, Number of layers:\t%d\n' %(model.n_layers_))
        fp.write('Best Network:\t')
        for l in model.intercepts_:
            fp.write('%d\t' %(l.size))
        fp.write('\nBiases : \n')
        i = 1
        for l in model.intercepts_:
        ...
        fp.write('Coefficients\n')
        i = 1
        for l in model.coefs_:
            fp.write('Layer:\t%d\n' %(i))
            np.savetxt(fp, l, fmt='%.6f', delimiter='\t')
            fp.write('\n')
            i += 1

    fp.close()
    #d = np.vstack([days, pd, ps, temp])
    d = np.reshape([15, 0.75, 100, 28], (1,-1))
    
    d = scaler.transform(d)

    pred = model_l.predict(d)
    print pred
    """ pred = scaler_y.inverse_transform(pred)
    #plt.plot(days, pred, label=activation)
    # plt.show()
